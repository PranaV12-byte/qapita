...
export default function BrainStats({ sources, passages, lastLintAt, coverage }: { sources: number; passages: number; lastLintAt: string | null; coverage: CoverageSummary }) {
  return <div className="v9-brain-stats">
    <div className="v9-brain-stats-grid grid grid-cols-2 gap-3">
    ...
    <p className="v9-brain-stats-note">Coverage is shown in the graph and reflects company-source connections.</p>
  </div>;
}

function Stat({ label, value }: { label: string; value: string }) {
  return <div className="v9-brain-stat"><div className="v9-brain-stat-value truncate text-lg font-semibold text-[#1f2937]">{value}</div><div className="v9-brain-stat-label mt-1 text-[10px] uppercase tracking-[0.08em] text-[#9ca3af]">{label}</div></div>;
}
